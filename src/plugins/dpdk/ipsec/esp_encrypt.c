...
extern vlib_node_registration_t dpdk_esp_proto_encrypt_node;

VLIB_NODE_FN (dpdk_esp_proto_encrypt_node) (vlib_main_t * vm,
				       vlib_node_runtime_t * node,
				       vlib_frame_t * from_frame)
{
  u32 n_left_from, *from, *to_next, next_index, thread_index;
  ...
  crypto_enqueue_ops (vm, cwm, dpdk_esp_proto_encrypt_node.index,
                     ESP_ENCRYPT_ERROR_ENQ_FAIL, numa, 1 /* encrypt */);
...
VLIB_REGISTER_NODE (dpdk_esp_proto_encrypt_node) = {
  ...
  .next_nodes =
    {
      //[ESP_PROTO_ENCRYPT_NEXT_DPDK_ESP] = "dpdk-esp4-encrypt",
