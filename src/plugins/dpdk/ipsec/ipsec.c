...
#define IPDEFTTL 64
  ...
  memcpy((uint8_t *)&tunnel->ipv4.src_ip,
                 (uint8_t *)&sa->tunnel.t_src.ip.ip4.as_u32, 4);
  memcpy((uint8_t *)&tunnel->ipv4.dst_ip,
                 (uint8_t *)&sa->tunnel.t_dst.ip.ip4.as_u32, 4);
...
crypto_create_crypto_op_pool (vlib_main_t * vm, u8 numa)
{
  ...
  mp = rte_mempool_create ((char *) pool_name, NUM_CRYPTO_MBUFS,
...
dpdk_ipsec_enable_disable (int is_enable)
{
  ...
  for (i = skip_master; i < n_mains; i++)
    vlib_node_set_state (vlib_get_main_by_index(i), node->index, is_enable != 0 ?
...
dpdk_ipsec_main_init (vlib_main_t * vm)
{
  ...
  /* Set this backend active by default */
  im->esp_current_backend = ~0;
  if (dcm->lookaside_proto_offload) {
	/* TODO support tun IPv6 proto offload
	 * Currently using non-proto offload nodes
	 */
	idx = ipsec_register_esp_backend (vm, im, "dpdk crypto lookaside",
					"dpdk-esp-proto-encrypt", "dpdk-esp4-encrypt-tun",
					"dpdk-esp-proto-decrypt", "dpdk-esp4-decrypt",
					"dpdk-esp6-encrypt", "dpdk-esp6-encrypt-tun",
					"dpdk-esp6-decrypt", "dpdk-esp6-decrypt",
					"error-drop",
	...
	idx = ipsec_register_esp_backend (vm, im, "dpdk crypto",
					"dpdk-esp4-encrypt", "dpdk-esp4-encrypt-tun",
					"dpdk-esp4-decrypt", "dpdk-esp4-decrypt",
					"dpdk-esp6-encrypt", "dpdk-esp6-encrypt-tun",
					"dpdk-esp6-decrypt", "dpdk-esp6-decrypt",
					"error-drop",
					dpdk_ipsec_check_support,
